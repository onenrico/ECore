...
public enum MinecraftVersion {
	...
	public boolean higher(MinecraftVersion mv) {
		return getN() >= mv.getN();
	}
	...
	public static MinecraftVersion getVersion() {
		...
		if (MinecraftVersion.version != MinecraftVersion.Unknown
				&& MinecraftVersion.version.higher(MinecraftVersion.MC1_8_R3)) {
			MessageUT.cmsg("&eNMS &fsupport &7'&e" + MinecraftVersion.version.name() + "&7' &bloaded!");
		} else {
			MessageUT.cmsg(
					"&7'&e" + MinecraftVersion.version.name() + "&7' &fcurrently &cnot supported &fplease tell Dev");
		}
		return MinecraftVersion.version;
	}
