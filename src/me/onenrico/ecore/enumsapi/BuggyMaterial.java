...
public enum BuggyMaterial {
	...
	private static HashMap<String, BuggyMaterial> cachedstring = new HashMap<>();
	private static HashMap<BuggyMaterial, Boolean> cachedunique = new HashMap<>();
	private static HashMap<BuggyMaterial, Material> cachedematerial = new HashMap<>();
	private static HashMap<Material, BuggyMaterial> cachedmaterial = new HashMap<>();

	private BuggyMaterial(final int data, String... m) {
		this.data = data;
		this.m = m;
	}
	...
	public static BuggyMaterial fromMaterial(final Material mat) {
		if (cachedmaterial.containsKey(mat)) {
			return cachedmaterial.get(mat);
		}
		for (BuggyMaterial em : values()) {
			if (isNewVersion()) {
		...
		return BuggyMaterial.STONE;
	}

	public static BuggyMaterial matching(final String name, final int data) {
		if (BuggyMaterial.cachedstring.containsKey(name + ":" + data)) {
			return BuggyMaterial.cachedstring.get(name + ":" + data);
		}
		for (BuggyMaterial mat : values()) {
	...
	public static BuggyMaterial fromString(String key) {
		if (key == null) {
			return null;
		}
		key = key.toUpperCase().trim();
		if (BuggyMaterial.cachedstring.containsKey(key)) {
			return BuggyMaterial.cachedstring.get(key);
		}
		if (key.startsWith("0") || key.startsWith("0:")) {
			BuggyMaterial.cachedstring.put(key, BuggyMaterial.AIR);
			return BuggyMaterial.AIR;
		}
		...
		if (isid) {
			for (Material m : Material.values()) {
				if (m.getId() == MathUT.strInt(key)) {
					...
					BuggyMaterial.cachedstring.put(key, fromMaterial(m));
					return fromString(key);
				}
			}
			BuggyMaterial.cachedstring.put(key, null);
			return null;
		} else {
			if (hasdata) {
				...
				if (ma != null) {
					BuggyMaterial.cachedstring.put(key, fromMaterial(ma));
					return fromMaterial(ma);
				}
				for (BuggyMaterial em : values()) {
					if (key.equals(em.toString())) {
						BuggyMaterial.cachedstring.put(key, em);
						return em;
					}
					for (String m : em.m) {
						if (key.equals(m)) {
	...
	public boolean isUnique() {
		if (BuggyMaterial.cachedunique.containsKey(this)) {
			return BuggyMaterial.cachedunique.get(this);
		}
		for (BuggyMaterial mat : values()) {
			if (mat.m.equals(m) && mat.data > 0) {
				BuggyMaterial.cachedunique.put(this, true);
				return true;
			}
		}
		BuggyMaterial.cachedunique.put(this, false);
		return false;
	}
}
