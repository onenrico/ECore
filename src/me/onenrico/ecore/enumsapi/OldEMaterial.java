...
public enum OldEMaterial {
	...
	static int newV;
	private static HashMap<String, OldEMaterial> cachedSearch;
	private static HashMap<OldEMaterial, Boolean> cachedUnique;

	static {
		OldEMaterial.newV = -1;
		OldEMaterial.cachedSearch = new HashMap<>();
		OldEMaterial.cachedUnique = new HashMap<>();
	}

	private OldEMaterial(final String m, final int data) {
		this.m = m;
		this.data = data;
	}
	...
	public static boolean isNewVersion() {
		if (OldEMaterial.newV == 0) {
			return false;
		}
		if (OldEMaterial.newV == 1) {
			return true;
		}
	...
	public boolean isSameMaterial(final ItemStack comp) {
		...
		final OldEMaterial xmat = fromMaterial(comp.getType());
		return isDamageable(xmat) && parseMaterial() == comp.getType();
	}

	public static OldEMaterial fromMaterial(final Material mat) {
		try {
			return valueOf(mat.toString());
		} catch (IllegalArgumentException e) {
			OldEMaterial[] values;
			for (int length = (values = values()).length, i = 0; i < length; ++i) {
				final OldEMaterial xmat = values[i];
	...
	public static OldEMaterial matching(final String name, final byte data) {
		if (OldEMaterial.cachedSearch.containsKey(String.valueOf(name.toUpperCase()) + "," + data)) {
			return OldEMaterial.cachedSearch.get(String.valueOf(name.toUpperCase()) + "," + data);
		}
		OldEMaterial[] values;
		for (int length = (values = values()).length, i = 0; i < length; ++i) {
			final OldEMaterial mat = values[i];
			if (name.toUpperCase().equals(mat.m) && (byte) mat.data == data) {
				OldEMaterial.cachedSearch.put(String.valueOf(mat.m) + "," + data, mat);
				return mat;
			}
	...
	public static OldEMaterial fromString(String key) {
		if (key != null) {
			key = key.toUpperCase();
		}
		OldEMaterial xmat = null;
		try {
	...
	public static boolean isDamageable(final OldEMaterial type) {
		if (type == null) {
			return false;
		}
	...
	public boolean isUnique(final OldEMaterial material) {
		if (OldEMaterial.cachedUnique.containsKey(material)) {
			return OldEMaterial.cachedUnique.get(material);
		}
		OldEMaterial[] values;
		for (int length = (values = values()).length, i = 0; i < length; ++i) {
			final OldEMaterial e = values[i];
			if (e.m.equalsIgnoreCase(material.m) && e.data > 0) {
				OldEMaterial.cachedUnique.put(material, true);
				return true;
			}
		}
		OldEMaterial.cachedUnique.put(material, false);
		return false;
	}
